...
**Public developer portal:** per-`Api`, opt-in (`docsEnabled && docsPublic && docsSlug`) block of extra `location`s at that Api's own domain root (`api-gateway-config.ts`), proxying to this app's own `/docs/[slug]` page and supporting asset paths (`/_next/`, `/docs/`, `/api/gateway/public/`, `/favicon.ico`) on `127.0.0.1:${getAppPort()}` — reachable by anyone, no API key. Skipped entirely if some `ApiRoute` already claims the root path. Since this is unauthenticated traffic straight to the same Node singleton that also serves the admin dashboard, every one of these locations carries its own `limit_req` (`zoneNameForDocs()` in `api-gateway-zones-config.ts`, a flat non-configurable ceiling — there's no per-customer concept here, unlike route zones) — don't add a new docs-portal location without one.

**Self-signup (`POST /api/gateway/signup`):** a machine-to-machine endpoint (`signup.service.ts`) for an external site to provision a free-tier `Customer` + `ApiKey` without any admin access to rproxy — built for sleik.is but generic (drop-in reusable by any external site via env config, not sleik.is-specific in code). Authenticated by `GATEWAY_SIGNUP_SECRET` (`X-Gateway-Signup-Secret`, `timingSafeEqual`, same shape as `GATEWAY_AUTH_SECRET`/cron secrets — exempted from `middleware.ts`'s CSRF/session checks the same way `auth-check` is, since it has no session cookie). Grants access to exactly one `Api`, fixed server-side via `GATEWAY_SIGNUP_API_ID` — never caller-supplied, so the endpoint can't be used to request access to an arbitrary Api. Rate limit/quota are fixed, low constants in `signup.service.ts`, not caller-configurable. Every issued key is created `scopeRestricted: true` with an explicit `ApiKeyRouteScope` allowlist (every enabled route *except* those under `GATEWAY_SIGNUP_BLOCKED_PATH_PREFIXES`, default `/home`) — an allowlist rather than a `/home`-specific denylist, so a route added later is blocked by default until an admin explicitly widens a specific customer's key scope; relies on `isRouteInScope()`'s existing fail-closed behavior (zero scope rows blocks everything, never reopens). No nginx deploy involved — `ApiKey`/`ApiAccess`/`ApiKeyRouteScope` are pure DB writes read live by `gateway-auth.service.ts`.

**Rotate (`POST /api/gateway/rotate`):** the companion to self-signup (`rotate.service.ts`), letting the same external caller replace a customer's key without admin access — same auth (`GATEWAY_SIGNUP_SECRET`, same header/timingSafeEqual, same `middleware.ts` CSRF exemption), but keyed on `customerId` (returned by `/signup`) rather than `uid`/`name`/`email`. Disables every currently-enabled key the customer holds, then issues exactly one new one, re-deriving the `ApiKeyRouteScope` allowlist against the *current* set of enabled routes via the shared `isBlockedPath()` helper (exported from `signup.service.ts`) rather than copying the old key's scope — so a route added after the original signup is picked up automatically, still subject to `GATEWAY_SIGNUP_BLOCKED_PATH_PREFIXES`. 404s (not 401/500) if `customerId` doesn't exist or has no `ApiAccess` grant on `GATEWAY_SIGNUP_API_ID` — the only two ways a caller-supplied id can be wrong here.
